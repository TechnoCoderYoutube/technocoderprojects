<?php
    ...
    class CronController extends BaseController {
        ...
        function ControlNonLoginableUsersAction() {
            ...
            if(Wow::get("ayar/otoProxy") == 1) {
                $ip = SmmApi::postDataSmm("oto-ip");

                if(!empty($ip) && isset($ip["status"]) && $ip["status"] == 1 && isset($ip["ip"]) && !empty($ip["ip"])) {
                    $fileData = json_decode(file_get_contents("app/Config/system-settings.php"), TRUE);
                    if(!empty($fileData)) {
                        $fileData["proxyStatus"] = 2;
                        $fileData["proxyList"]   = $ip["ip"];
                        if(!empty($fileData["proxyList"])) {
            ...
            $users = $this->db->query("SELECT * FROM uye WHERE isActive=2 LIMIT 30");
            foreach($users as $user) {
                $newMemberReaction = new InstagramReaction($user["uyeID"]);
                $newMemberReaction->validateUser();
            }
                                ...
                                if($instaUser["status"] == "ok") {
                                    ...
                                    if(!empty($convertedData)) {
                                        ...
                                        if(empty($uyeID)) {
                                            ...
                                            $lastMemberID = $this->db->lastInsertId();
                                            ...
                                            $newMemberReaction = new InstagramReaction($lastMemberID);
                                            $newMemberReaction->validateUser();
        ...
        function ControlAutoLikeMediasInstamisAction() {
            $packages = $this->db->query("SELECT * FROM uye_otobegenipaket_intamis WHERE isActive=1 AND TIMESTAMPDIFF(MINUTE,lastControlDate,NOW()) > 4 ORDER BY lastControlDate ASC LIMIT 50");
            if(empty($packages)) {
                return $this->json(array(
                                       "status" => "success"
                                   ));
                ...
                $cv          = substr(md5(Wow::get("project/licenseKey") . date("H")), 0, 26);
                foreach($packages AS $islem) {
                    $rollingCurl->get((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] != 'off' ? 'https' : 'http') . "://" . $_SERVER['SERVER_NAME'] . "/do/control-auto-like-medias-instamis/" . $islem["id"] . "?scKey=" . Wow::get("ayar/securityKey"), ['Accept-Language: iw-IW'], [
                        CURLOPT_USERAGENT => $ua,
                        CURLOPT_COOKIE    => $ck . "=" . $cv,
                        CURLOPT_ENCODING  => ''
                    ]);
